 ...
 */
export const GET = withAdmin<UserListResponse>(async (request: NextRequest, { user: requester }) => {
  try {
    ...
    const canSeeEmail = requester.role === 'ust_yetkili' || requester.role === 'owner';
    ...
    // Arama filtresi - owner/ust_yetkili email'de de arayabilir
    ...
    if (search) {
      if (canSeeEmail) {
        ...
        whereConditions.username = { contains: search, mode: 'insensitive' };
      }
    }
