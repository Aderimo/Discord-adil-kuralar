...
/**
 * Komut adı ile komut getirir (örn: "/mute", "s!mute", "h!ban")
 */
export function getCommandByName(command: string): CommandDefinition | null {
  const lower = command.toLowerCase();
  const commands = loadCommands();
  // Önce tam eşleşme dene
  const exact = commands.find((c) => c.command.toLowerCase() === lower);
  if (exact) return exact;
  // Slash prefix ekleyerek dene (geriye uyumluluk)
  if (!lower.startsWith('/') && !lower.includes('!')) {
    return commands.find((c) => c.command.toLowerCase() === `/${lower}`) || null;
  }
  return null;
}
