 ...
 */
const validRoleArbitrary = fc.constantFrom('none', 'reg', 'gm', 'ust_yetkili');
            ...
            async (ipAddress, oldRole, newRole, actionType) => {
              ...
              const targetUser = await prisma.user.create({
                data: {
                  ...
                  passwordHash,
                  status: 'pending',
                  role: oldRole as 'none' | 'reg' | 'gm' | 'ust_yetkili',
                },
              });
              ...
              if (actionType === 'role_change') {
                log = await logRoleChange(
                  adminUser.id,
                  targetUser.id,
                  oldRole as 'none' | 'reg' | 'gm' | 'ust_yetkili',
                  newRole as 'none' | 'reg' | 'gm' | 'ust_yetkili',
                  ipAddress,
                  targetUser.username
                );
...
describe('Property Tests: Activity Logging Completeness (Property 7)', () => {
   ...
   * Geçerli yetki seviyesi oluşturan arbitrary
   */
  const roleArbitrary = fc.constantFrom('none', 'reg', 'gm', 'ust_yetkili') as fc.Arbitrary<'none' | 'reg' | 'gm' | 'ust_yetkili'>;
