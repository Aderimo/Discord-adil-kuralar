  ...
  it(
    ...
    async () => {
      await fc.assert(
        fc.property(protectedRouteArbitrary, (route) => {
          // Admin kullanıcı
          const adminUser = createTestUser('approved', 'gm');
    ...
    async () => {
      await fc.assert(
        fc.property(adminRouteArbitrary, (route) => {
          // Mod kullanıcı
          const modUser = createTestUser('approved', 'reg');
...
const ADMIN_EXPECTED_PERMISSIONS: Permission[] = [
  ...
  'VIEW_USERS',
  'EDIT_CONTENT',
  // EDIT_USERS artık ust_yetkili+ gerektiriyor (gm için mevcut değil)
];
...
describe('Property Tests: Property 6 - RBAC Permission Enforcement', () => {
    ...
    async () => {
      await fc.assert(
        fc.property(
          ...
          (permission) => {
            const hasAccess = hasPermission('reg', permission);
    ...
    async () => {
      await fc.assert(
        fc.property(
          fc.constantFrom(...ALL_PERMISSIONS),
          (permission) => {
            const hasAccess = hasPermission('gm', permission);
    ...
    async () => {
      await fc.assert(
        fc.property(
          fc.constantFrom(...ALL_PERMISSIONS),
          (permission) => {
            const modHas = hasPermission('reg', permission);
            const adminHas = hasPermission('gm', permission);
  ...
  it(
    'Property 6.10: EDIT_TEMPLATES izni sadece üst yetkililere ait olmalı',
    async () => {
      const modHas = hasPermission('reg', 'EDIT_TEMPLATES');
      const adminHas = hasPermission('gm', 'EDIT_TEMPLATES');
