...
// Geçerli roller
const VALID_ROLES: UserRole[] = ['reg', 'gm', 'ust_yetkili'];
 ...
 */
const validRoleArbitrary = fc.constantFrom<UserRole>('reg', 'gm', 'ust_yetkili');
...
describe('Property Tests: Admin - User Filtering Correctness', () => {
  ...
  const FILTER_ROLES: UserRole[] = ['reg', 'gm', 'ust_yetkili'];
   ...
   */
  const roleWithNoneArbitrary = fc.constantFrom<UserRole>('none', 'reg', 'gm', 'ust_yetkili');
      ...
      try {
        await fc.assert(
          fc.asyncProperty(
            ...
            createFilterTestEmailArbitrary(),
            validPasswordArbitrary,
            fc.constantFrom<UserRole>('reg', 'gm', 'ust_yetkili'), // Filtrelenecek rol
      ...
      try {
        await fc.assert(
          fc.asyncProperty(
            ...
            validPasswordArbitrary,
            statusArbitrary,
            fc.constantFrom<UserRole>('reg', 'gm', 'ust_yetkili'),
            async (username, email, password, status, role) => {
              const passwordHash = await hashPassword(password);
        ...
        await fc.assert(
          fc.asyncProperty(
            ...
            async (pendingCount, approvedCount, rejectedCount, baseUsername, baseEmail, password) => {
              ...
              // Önceki test çalışmalarından kalan aynı pattern'deki kullanıcıları temizle
              await prisma.user.deleteMany({
        ...
        await fc.assert(
          fc.asyncProperty(
            ...
            async (selectedCount, unselectedCount, baseUsername, baseEmail, password, action, uniqueNum) => {
              ...
              const newRole = action === 'approve' ? 'reg' : 'none';
